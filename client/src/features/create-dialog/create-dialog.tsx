...
export const CreateDialog = () => {
  const [text, handleChangeText, handleSubmitTextForm, { recipientId }] =
    useUnit([
      model.$messageText,
      model.messageTextChanged,
      model.textFormSubmitted,
      routes.app.createDialog.$query,
    ]);

  return (
    <form
      onSubmit={(e) => (e.preventDefault(), handleSubmitTextForm(recipientId))}
      className="flex h-100 flex-1 gap-15 border-t-2 border-blue p-10"
    >
      <textarea
        value={text}
